// Generated from ./src/grammar/Decaf.g4 by ANTLR 4.9.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by DecafParser.
export default class DecafListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by DecafParser#programInit.
	enterProgramInit(ctx) {
	}

	// Exit a parse tree produced by DecafParser#programInit.
	exitProgramInit(ctx) {
	}


	// Enter a parse tree produced by DecafParser#declarationStruct.
	enterDeclarationStruct(ctx) {
	}

	// Exit a parse tree produced by DecafParser#declarationStruct.
	exitDeclarationStruct(ctx) {
	}


	// Enter a parse tree produced by DecafParser#declarationVar.
	enterDeclarationVar(ctx) {
	}

	// Exit a parse tree produced by DecafParser#declarationVar.
	exitDeclarationVar(ctx) {
	}


	// Enter a parse tree produced by DecafParser#declarationMethod.
	enterDeclarationMethod(ctx) {
	}

	// Exit a parse tree produced by DecafParser#declarationMethod.
	exitDeclarationMethod(ctx) {
	}


	// Enter a parse tree produced by DecafParser#varDecl.
	enterVarDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#varDecl.
	exitVarDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#arrDecl.
	enterArrDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#arrDecl.
	exitArrDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#structDecl.
	enterStructDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#structDecl.
	exitStructDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#intVar.
	enterIntVar(ctx) {
	}

	// Exit a parse tree produced by DecafParser#intVar.
	exitIntVar(ctx) {
	}


	// Enter a parse tree produced by DecafParser#charVar.
	enterCharVar(ctx) {
	}

	// Exit a parse tree produced by DecafParser#charVar.
	exitCharVar(ctx) {
	}


	// Enter a parse tree produced by DecafParser#booleanVar.
	enterBooleanVar(ctx) {
	}

	// Exit a parse tree produced by DecafParser#booleanVar.
	exitBooleanVar(ctx) {
	}


	// Enter a parse tree produced by DecafParser#structVar.
	enterStructVar(ctx) {
	}

	// Exit a parse tree produced by DecafParser#structVar.
	exitStructVar(ctx) {
	}


	// Enter a parse tree produced by DecafParser#structDeclarationVar.
	enterStructDeclarationVar(ctx) {
	}

	// Exit a parse tree produced by DecafParser#structDeclarationVar.
	exitStructDeclarationVar(ctx) {
	}


	// Enter a parse tree produced by DecafParser#voidVar.
	enterVoidVar(ctx) {
	}

	// Exit a parse tree produced by DecafParser#voidVar.
	exitVoidVar(ctx) {
	}


	// Enter a parse tree produced by DecafParser#methodDecl.
	enterMethodDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#methodDecl.
	exitMethodDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#intMethod.
	enterIntMethod(ctx) {
	}

	// Exit a parse tree produced by DecafParser#intMethod.
	exitIntMethod(ctx) {
	}


	// Enter a parse tree produced by DecafParser#charMethod.
	enterCharMethod(ctx) {
	}

	// Exit a parse tree produced by DecafParser#charMethod.
	exitCharMethod(ctx) {
	}


	// Enter a parse tree produced by DecafParser#booleanMethod.
	enterBooleanMethod(ctx) {
	}

	// Exit a parse tree produced by DecafParser#booleanMethod.
	exitBooleanMethod(ctx) {
	}


	// Enter a parse tree produced by DecafParser#voidMethod.
	enterVoidMethod(ctx) {
	}

	// Exit a parse tree produced by DecafParser#voidMethod.
	exitVoidMethod(ctx) {
	}


	// Enter a parse tree produced by DecafParser#idParam.
	enterIdParam(ctx) {
	}

	// Exit a parse tree produced by DecafParser#idParam.
	exitIdParam(ctx) {
	}


	// Enter a parse tree produced by DecafParser#idArrParam.
	enterIdArrParam(ctx) {
	}

	// Exit a parse tree produced by DecafParser#idArrParam.
	exitIdArrParam(ctx) {
	}


	// Enter a parse tree produced by DecafParser#voidParam.
	enterVoidParam(ctx) {
	}

	// Exit a parse tree produced by DecafParser#voidParam.
	exitVoidParam(ctx) {
	}


	// Enter a parse tree produced by DecafParser#intParam.
	enterIntParam(ctx) {
	}

	// Exit a parse tree produced by DecafParser#intParam.
	exitIntParam(ctx) {
	}


	// Enter a parse tree produced by DecafParser#charParam.
	enterCharParam(ctx) {
	}

	// Exit a parse tree produced by DecafParser#charParam.
	exitCharParam(ctx) {
	}


	// Enter a parse tree produced by DecafParser#booleanParam.
	enterBooleanParam(ctx) {
	}

	// Exit a parse tree produced by DecafParser#booleanParam.
	exitBooleanParam(ctx) {
	}


	// Enter a parse tree produced by DecafParser#blockDecl.
	enterBlockDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#blockDecl.
	exitBlockDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#ifStmt.
	enterIfStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#ifStmt.
	exitIfStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#ifElseStmt.
	enterIfElseStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#ifElseStmt.
	exitIfElseStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#whileStmt.
	enterWhileStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#whileStmt.
	exitWhileStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#returnExprStmt.
	enterReturnExprStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#returnExprStmt.
	exitReturnExprStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#returnVoidStmt.
	enterReturnVoidStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#returnVoidStmt.
	exitReturnVoidStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#methodStmt.
	enterMethodStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#methodStmt.
	exitMethodStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#blockStmt.
	enterBlockStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#blockStmt.
	exitBlockStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#assignmentStmt.
	enterAssignmentStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#assignmentStmt.
	exitAssignmentStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#expressionStmt.
	enterExpressionStmt(ctx) {
	}

	// Exit a parse tree produced by DecafParser#expressionStmt.
	exitExpressionStmt(ctx) {
	}


	// Enter a parse tree produced by DecafParser#idLocation.
	enterIdLocation(ctx) {
	}

	// Exit a parse tree produced by DecafParser#idLocation.
	exitIdLocation(ctx) {
	}


	// Enter a parse tree produced by DecafParser#arrLocation.
	enterArrLocation(ctx) {
	}

	// Exit a parse tree produced by DecafParser#arrLocation.
	exitArrLocation(ctx) {
	}


	// Enter a parse tree produced by DecafParser#idDotLocation.
	enterIdDotLocation(ctx) {
	}

	// Exit a parse tree produced by DecafParser#idDotLocation.
	exitIdDotLocation(ctx) {
	}


	// Enter a parse tree produced by DecafParser#idArrDotLocation.
	enterIdArrDotLocation(ctx) {
	}

	// Exit a parse tree produced by DecafParser#idArrDotLocation.
	exitIdArrDotLocation(ctx) {
	}


	// Enter a parse tree produced by DecafParser#methodCallExpr.
	enterMethodCallExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#methodCallExpr.
	exitMethodCallExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#parExpr.
	enterParExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#parExpr.
	exitParExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#notExpr.
	enterNotExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#notExpr.
	exitNotExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#eqOpExpr.
	enterEqOpExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#eqOpExpr.
	exitEqOpExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#relOpExpr.
	enterRelOpExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#relOpExpr.
	exitRelOpExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#locationExpr.
	enterLocationExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#locationExpr.
	exitLocationExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#condOpExpr.
	enterCondOpExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#condOpExpr.
	exitCondOpExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#literalExpr.
	enterLiteralExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#literalExpr.
	exitLiteralExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#negativeExpr.
	enterNegativeExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#negativeExpr.
	exitNegativeExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#secondArithOpExpr.
	enterSecondArithOpExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#secondArithOpExpr.
	exitSecondArithOpExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#firstArithOpExpr.
	enterFirstArithOpExpr(ctx) {
	}

	// Exit a parse tree produced by DecafParser#firstArithOpExpr.
	exitFirstArithOpExpr(ctx) {
	}


	// Enter a parse tree produced by DecafParser#methodCallDecl.
	enterMethodCallDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#methodCallDecl.
	exitMethodCallDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#argDecl.
	enterArgDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#argDecl.
	exitArgDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#intLiteral.
	enterIntLiteral(ctx) {
	}

	// Exit a parse tree produced by DecafParser#intLiteral.
	exitIntLiteral(ctx) {
	}


	// Enter a parse tree produced by DecafParser#charLiteral.
	enterCharLiteral(ctx) {
	}

	// Exit a parse tree produced by DecafParser#charLiteral.
	exitCharLiteral(ctx) {
	}


	// Enter a parse tree produced by DecafParser#boolLiteral.
	enterBoolLiteral(ctx) {
	}

	// Exit a parse tree produced by DecafParser#boolLiteral.
	exitBoolLiteral(ctx) {
	}


	// Enter a parse tree produced by DecafParser#charLiteralDecl.
	enterCharLiteralDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#charLiteralDecl.
	exitCharLiteralDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#numLiteral.
	enterNumLiteral(ctx) {
	}

	// Exit a parse tree produced by DecafParser#numLiteral.
	exitNumLiteral(ctx) {
	}


	// Enter a parse tree produced by DecafParser#trueLiteral.
	enterTrueLiteral(ctx) {
	}

	// Exit a parse tree produced by DecafParser#trueLiteral.
	exitTrueLiteral(ctx) {
	}


	// Enter a parse tree produced by DecafParser#falseLiteral.
	enterFalseLiteral(ctx) {
	}

	// Exit a parse tree produced by DecafParser#falseLiteral.
	exitFalseLiteral(ctx) {
	}


	// Enter a parse tree produced by DecafParser#idDecl.
	enterIdDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#idDecl.
	exitIdDecl(ctx) {
	}


	// Enter a parse tree produced by DecafParser#numDecl.
	enterNumDecl(ctx) {
	}

	// Exit a parse tree produced by DecafParser#numDecl.
	exitNumDecl(ctx) {
	}



}